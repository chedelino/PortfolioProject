...
SELECT
      ...
      Max((dt.total_cases)/dt.population) AS PercentPopulationInfected,
      MAX(cast(dt.total_deaths AS INT)) AS TotalDeathCounts,
      Max(cast(dt.total_cases AS BIGINT)) / dt.population AS InfectionRate,
      Max(CONVERT(int, vacc.new_vaccinations)) AS TotalVaccinated,
      Max(CONVERT(int, vacc.new_vaccinations)) / dt.population AS PercentVaccinated
...
OR dt.continent = 'South America'
Group by dt.location, dt.population



SELECT
      date, location,population,  MAX(total_cases) AS TotalCases, Max((total_cases)/population) AS PercentPopulationInfected
FROM PortfolioProject.dbo.CovidDeaths
Where continent Like '%North America%'
OR continent LIKE '%South America%'
...
OR deat.continent ='South America'
)
